...
(function() {
  ...
  $(document).ready(function() {
    var delay, fieldSearchByName, me2, searchRequestError, searchRequestToServer;
    ...
    $('#search-by-name-field').keyup(delay(function(e) {
      var data, last, result_data;
      ...
      if (data.length >= 3) {
        if (indexOf.call(data, "|") >= 0) {
          ...
          if (last.length >= 3) {
            return fieldSearchByName(result_data);
          }
        } else {
          return fieldSearchByName([data]);
        }
      }
    }));
    fieldSearchByName = function(stringData) {
      var send_data, url;
      url = "/employee/search/";
      send_data = {
        limit: 20,
      ...
      return searchRequestToServer(url, send_data, function(data, textStatus, jqXHR) {
        return console.log(data);
      });
    };
    searchRequestToServer = function(url, requestData, requestDone) {
      var request;
      request = $.ajax({
        url: url,
        type: "GET",
    ...
    return searchRequestError = function(jqXHR, textStatus, errorThrown) {
